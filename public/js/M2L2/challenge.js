...
window.onload = async () => {
    const user = await getCurrentUserFromFirestore();
    if (user.user_stageCleared.includes("module3Lecture1")) {
        curr_script = 1;
    }
};
...
const decisionTreeThompsonSampling = async () => {
    ...
    const handleUserChoice = async (choice, question, currEnemy, userChar) => {
        ...
        if (userChar.user_bruhs[curr_bruh].bruh_hp <= 0) {
            ...
            await updateUserDataStageCleared(userChar, 'module3Lecture1');
    ...
    if (currUser.user_stageCleared.includes("module3Lecture1")) {
