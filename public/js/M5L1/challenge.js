...
window.onload = async () => {
    const user = await getCurrentUserFromFirestore();
    if (user.user_stageCleared.includes("module5Lecture1")) {
        curr_script = 1;
    }
};
...
const decisionTreeThompsonSampling = async () => {
    ...
    enemy.forEach((enemies) => {
        if (enemies.enememe_name == 'Hates black') {
            currEnemy = enemies;
        }
    })
    ...
    const handleUserChoice = async (choice, question, currEnemy, userChar) => {
        ...
        if (userChar.user_bruhs[curr_bruh].bruh_hp <= 0) {
            ...
            await updateUserDataStageCleared(userChar, 'module5Lecture1');
    ...
    if (currUser.user_stageCleared.includes("module5Lecture1")) {
