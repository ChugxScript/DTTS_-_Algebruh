...
takeChallBTN.addEventListener('click', function() {
    ...
    curr_enemy_name = '3rd year college';
    isSave = true;
    decisionTreeThompsonSampling();
})

let isPreBattle = true;
...
window.onload = async () => {
    ...
    if (user.user_stageCleared.includes("module1Lecture1")) {
        curr_script = 9;
    }
};
...
const decisionTreeThompsonSampling = async () => {
    ...
    enemy.forEach((enemies) => {
        if (enemies.enememe_name == curr_enemy_name) {
            currEnemy = enemies;
        }
    })
    ...
    const handleUserChoice = async (choice, question, currEnemy, userChar) => {
        ...
        if (userChar.user_bruhs[curr_bruh].bruh_hp <= 0) {
            ...
            if(isSave) {
                await updateUserDataStageCleared(userChar, 'module1Lecture1');
    ...
    if (currUser.user_stageCleared.includes("module1Lecture1")) {
